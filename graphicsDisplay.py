...
class PacmanGraphics:
    ...
    def drawStaticObjects(self, state):
        ...
        self.lakes = self.drawLakes(layout.lakes)
        refresh()

    def drawLakes(self, lakes):
        lakeImages = {}
        for lake in lakes.asList():
            (screen_x, screen_y) = self.to_screen(lake)
            point = square((screen_x, screen_y),
                     0.5 * self.gridSize,
                     color = WALL_COLOR,
                     filled = 1, behind=2)
            lakeImages[lake] = point
        return lakeImages
